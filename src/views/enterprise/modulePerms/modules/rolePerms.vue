    ...
    <a-card :bordered="false">
      ...
      <a-table :loading="loading" :size="tableSize" rowKey="roleId" :columns="columns" :data-source="list" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :pagination="false">
        ...
        <span slot="operation" slot-scope="text, record">
          <a @click="$refs.MenuTree.handleUpdate(record, '角色')"> <a-icon type="edit" />添加模块 </a>
        </span>
      </a-table>
      <!-- 分页 -->
      <a-pagination
...
<script>
import { listRoleAll } from '/src/api/system/role.js'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listRoleAll(this.addDateRange(this.queryParam, this.dateRange)).then(response => {
