...
<script>
import { uploadAvatar } from '/src/api/system/user.js'
      ...
      if (type === 'blob') {
        this.$refs.cropper.getCropBlob(data => {
          ...
          uploadAvatar(formData).then(response => {
            this.open = false
            this.$store.commit('SET_AVATAR', import.meta.env.VITE_APP_BASE_API + response.imgUrl)
            _this.$message.success('上传成功')
            _this.$emit('ok', import.meta.env.VITE_APP_BASE_API + response.imgUrl)
            _this.visible = false
          })
        })
      } else {
