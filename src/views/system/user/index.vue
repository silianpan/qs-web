      ...
      <a-row :gutter="24">
        ...
        <a-col :span="20">
          ...
          <div class="table-operations">
            <a-button type="primary" @click="$refs.createForm.handleAdd()" v-hasPermi="['system:user:add']"> <a-icon type="plus" />新增 </a-button>
            <a-button type="primary" :disabled="single" @click="$refs.createForm.handleUpdate(undefined, ids)" v-hasPermi="['system:user:edit']"> <a-icon type="edit" />修改 </a-button>
            <a-button type="danger" :disabled="multiple" @click="handleDelete" v-hasPermi="['system:user:remove']"> <a-icon type="delete" />删除 </a-button>
            <a-button type="dashed" @click="$refs.importExcel.importExcelHandleOpen()" v-hasPermi="['system:user:import']"> <a-icon type="import" />导入 </a-button>
            <a-button type="primary" @click="handleExport" v-hasPermi="['system:user:export']"> <a-icon type="download" />导出 </a-button>
            <a-button type="primary" @click="exportDing" v-hasPermi="['system:user:export']"> <a-icon type="download" />导入钉钉数据 </a-button>
            <a-button type="dashed" shape="circle" :loading="loading" :style="{ float: 'right' }" icon="reload" @click="getList" />
          </div>
          <!-- 增加修改 -->
          <!-- 创建/编辑用户,单独封装了组件 -->
          <create-form ref="createForm" :deptOptions="deptOptions" :statusOptions="statusOptions" :sexOptions="sexOptions" @ok="getList" @select-tree="getTreeselect" />
          <!-- 修改密码抽屉 -->
          <reset-password ref="resetPassword" />
          <!-- 上传文件 -->
          <import-excel ref="importExcel" @ok="getList" />
          <!-- 数据展示 -->
          <a-table :loading="loading" :size="tableSize" rowKey="userId" :columns="columns" :data-source="list" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :pagination="false">
...
<script>
import { listUser, delUser, exportUser, changeUserStatus, importdingding } from '/src/api/system/user.js'
import { treeselect } from '/src/api/system/dept.js'
