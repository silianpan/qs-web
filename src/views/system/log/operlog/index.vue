  ...
  <page-header-wrapper :breadcrumb="$store.getters.breadcrumb">
    <a-card :bordered="false">
      ...
      <div class="table-operations">
        <a-button type="danger" :disabled="multiple" @click="handleDelete" v-hasPermi="['monitor:operlog:remove']"> <a-icon type="delete" />删除 </a-button>
        <a-button type="danger" @click="handleClean" v-hasPermi="['monitor:operlog:remove']"> <a-icon type="delete" />清空 </a-button>
      ...
      <!-- 数据展示 -->
      <a-table :loading="loading" :size="tableSize" rowKey="operId" :columns="columns" :data-source="list" :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }" :pagination="false">
        ...
        <span slot="operation" slot-scope="text, record">
          <a @click="$refs.viewForm.handleView(record)" v-hasPermi="['monitor:operlog:query']"> <a-icon type="eye" />详细 </a>
        </span>
      </a-table>
      <!-- 分页 -->
      <a-pagination
...
<script>
import { list, delOperlog, cleanOperlog, exportOperlog } from '/src/api/system/log/operlog.js'
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$antdconfirm({
        ...
        onOk() {
          return exportOperlog(that.queryParam).then(response => {
