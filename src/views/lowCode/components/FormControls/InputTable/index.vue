      ...
      <el-table-column v-for="(head, cindex) in tableData" :key="head.formId" :min-width="head['min-width']" :prop="head.vModel">
        ...
        <template slot-scope="scope">
          ...
          <template v-else-if="head.tag === 'el-upload'">
            <el-upload v-bind="getConfById(head.formId)" :on-success="res => onUploadSuccess(res, tableFormData[scope.$index][cindex])" @mouseleave.native="hideUploadList" @mouseenter.native="showUploadList">
              <span slot="default">
                已上传
...
<script>
import { useableProps } from './config.js'
  ...
  methods: {
    ...
    reset() {
      this.tableData.map(t => {
        var index = this.tableFormData[0].findIndex(c => c.vModel === t.vModel)
        if (index === -1) return
