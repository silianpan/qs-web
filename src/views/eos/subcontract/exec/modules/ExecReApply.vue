...
<script>
import {getExec, submitReApply} from '/src/api/eos/subcontract/exec'
import ViewVerify from '/src/components/Activiti/ViewVerify/index.vue'
import ExecForm from './ExecForm.vue'
  ...
  methods: {
    getProjectInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getExec(this.todoItem.formId).then(res => {
          this.updateRow = res.data
          this.busComp = ExecForm
        })
      }
    },
    close() {
    ...
    handleReApply() {
      ...
      this.$refs.deposit.$refs.formTable.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          fd.type = '工程量'
          fd.sealAmountListExecs = this.$refs.deposit.materialProcurement
          const data = {
            busForm: fd,
