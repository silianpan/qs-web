...
<script>
import {getEmployee, submitReApply} from '/src/api/eos/subcontract/employee'
  ...
  methods: {
    getProjectInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getEmployee(this.todoItem.formId).then(res => {
          this.updateRow = res.data
          this.busComp = EmployeeForm
        })
      }
    },
    close() {
    ...
    handleReApply() {
      ...
      this.$refs.deposit.$refs.formTable.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          fd.type = '零星用工'
          fd.sealEmployees = this.$refs.deposit.materialProcurement
          const data = {
            busForm: fd,
