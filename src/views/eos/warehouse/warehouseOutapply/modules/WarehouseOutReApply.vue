...
<template>
  <div>
    <component ref="WarehouseOutApply" :is="busComp" :updateRow="updateRow" :isUpdate="true" />
...
import WarehouseOutapplyForm from './WarehouseOutapplyForm'
import { getWarehouseOutapply, reApply } from '/src/api/eos/warehouse/warehouseOutapply.js'
  ...
  methods: {
    getApplyInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getWarehouseOutapply(this.todoItem.formId).then(({ data }) => {
          this.updateRow = data
          this.busComp = WarehouseOutapplyForm
        })
      }
    },
    close() {
    ...
    handleReApply() {
      ...
      this.$refs.WarehouseOutApply.$refs.formTable.$refs.ruleForm.validate(valid => {
        if (valid) {
          const fd = { ...this.todoItem }
          fd.form = { ...this.$refs.WarehouseOutApply.formData }
          fd.form.warehouseOutDetailList = [...this.$refs.WarehouseOutApply.tableData]
          const data = {
            busForm: fd.form,
            taskId: fd.taskId,
