<!-- 划拨申请明细表单 -->
<template>
  <s-form-table ok :okText="'保存'" :queryDictKeys="dictKeys" :formTitle="'划拨申请明细表单'" :formItems="formItems" :formData="formData" :rules="formRules" @submit="handleSubmit" @cancel="handleCancel">
...
import { addWarehouseTransferDetail, updateWarehouseTransferDetail } from '/src/api/eos/warehouse/warehouseTransferDetail'

export default {
  name: 'WarehouseTransferDetailForm',
  components: {
    SFormTable
  },
  props: {
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        warehouseTransferApplyId: { required: true, message: '请输入', trigger: 'blur' },
        ...
        transferNum: { required: true, message: '请输入', trigger: 'blur' }
      }
    }
  },
  created() {
  ...
  methods: {
    handleSubmit() {
      ...
      if (this.isUpdate && !this.isEmpty(fd.id)) {
        updateWarehouseTransferDetail(fd).then(res => {
          this.$message.success('修改成功', 3)
          this.$emit('ok')
          this.$emit('close')
        })
      } else {
        addWarehouseTransferDetail(fd).then(res => {
