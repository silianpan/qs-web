...
<template>
  <div>
    <component ref="WarehouseInApply" :is="busComp" :updateRow="updateRow" :isUpdate="true" />
...
import WarehouseInApplyForm from './WarehouseInApplyForm.vue'
import { getWarehouseInApply, reApply } from '/src/api/eos/warehouse/warehouseInApply.js'
import { finishTask } from '/src/api/activiti/process'

export default {
  name: 'WarehouseInReApply',
  components: {
    ViewVerify,
    WarehouseInApplyForm
  },
  props: {
    todoItem: {
      type: Object,
  ...
  methods: {
    getApplyInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getWarehouseInApply(this.todoItem.formId).then(({ data }) => {
          this.updateRow = data
          this.busComp = WarehouseInApplyForm
        })
      }
    },
    close() {
    ...
    handleReApply() {
      ...
      this.$refs.WarehouseInApply.$refs.formTable.$refs.ruleForm.validate(valid => {
        if (valid) {
          const fd = { ...this.todoItem }
          fd.form = { ...this.$refs.WarehouseInApply.formData }
          fd.form.warehouseInDetailList = [...this.$refs.WarehouseInApply.tableData]
          const data = {
            busForm: fd.form,
            taskId: fd.taskId,
