...
export default {
    ...
    return {
      ...
      formItems: [
        ...
        },
        {
          ...
          prop: 'status',
          customRender: (text, record) => {
            return text === '审批通过' ? <a-tag color="orange">{text}</a-tag> : text === '审批未通过' ? <a-tag color="red">{text}</a-tag> : <a-tag color="blue">{text}</a-tag>
          }
        },
        {
          label: '附件',
  ...
  created() {
    this.sealFundOutcomes = this.row.sealFundOutcomes ? this.row.sealFundOutcomes : []
    this.sealFundIncomes = this.row.sealFundIncomes ? this.row.sealFundIncomes : []
    // 查询字典
