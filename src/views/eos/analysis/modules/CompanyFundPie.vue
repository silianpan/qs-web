<!-- 公司资金计划饼状图 -->
<template>
  <a-card title="资金计划汇总收支占比" :bordered="false" :headStyle="{ fontSize: '20px', fontWeight: 'bold', textAlign: 'center' }">
    <div id="containerCompanyPie"></div>
  </a-card>
</template>
...
import { companyPlanTablePie } from '/src/api/eos/analysis/analyst.js'
...
export default {
  ...
  methods: {
    queryData(isUpdate) {
      ...
      companyPlanTablePie(Object.assign({}, QueryParams)).then(res => {
        if (isUpdate) {
    ...
    initData(data) {
      this.chart = new Pie('containerCompanyPie', {
        data: data,
        appendPadding: 10,
        angleField: 'money',
        colorField: 'kind',
