...
<template>
  <div>
    <component ref="laborApply" :is="busComp" :updateRow="updateRow" :isUpdate="true" />
...
import { reApply, getLabor } from '/src/api/eos/cost/labor.js'
  ...
  methods: {
    getMemApplyInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getLabor(this.todoItem.formId).then(({ data }) => {
          this.updateRow = data
          this.busComp = LaborForm
        })
      }
    },
    close() {
    ...
    handleReApply() {
      ...
      const loanVo = { ...this.$refs.laborApply.formData }
      loanVo.date = loanVo.createTime
      loanVo.sealLoanDetails = this.$refs.laborApply.cacheData
      const data = {
        busForm: loanVo,
