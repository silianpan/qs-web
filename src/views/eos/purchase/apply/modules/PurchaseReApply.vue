...
<template>
  <div>
    <component ref="purchaseApply" :is="busComp" :updateRow="updateRow" :isUpdate="true" />
...
import { getDetail, reApply } from '/src/api/eos/purchase/purchase.js'
import { finishTask } from '/src/api/activiti/process'

export default {
  name: 'PurchaseReApply',
  components: {
    ViewVerify,
    ApplyForm
  },
  ...
  methods: {
    getApplyInfo() {
      if (this.todoItem && this.todoItem.formId) {
        getDetail(this.todoItem.formId).then(({ data }) => {
          this.updateRow = data
    ...
    handleReApply() {
      ...
      this.$refs.purchaseApply.$refs.purchaseApplyForm.$refs.ruleForm.validate(valid => {
        if (valid) {
          const fd = { ...this.todoItem }
          fd.form = { ...this.$refs.purchaseApply.formData }
          fd.form.purchaseApplyDetail = [...this.$refs.purchaseApply.inTableData]
          const data = {
            busForm: fd.form,
            taskId: fd.taskId,
