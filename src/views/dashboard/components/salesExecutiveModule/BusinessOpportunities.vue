...
<template>
  <a-card class="card-container card-container-height" :bordered="false" :headStyle="{ border: 'none' }" :bodyStyle="{ padding: 0, overflow: 'hidden' }">
    ...
    <div id="containerBusinessOpportunityBOX"></div>
  </a-card>
</template>

<script>
import { Pie, measureTextWidth } from '@antv/g2plot'
import { opportunitiesToFollowUp } from '/src/api/dashboard/SalesExecutive.js'
...
export default {
  ...
  methods: {
    queryData() {
      // todo api请求
      opportunitiesToFollowUp().then(res => {
        // console.log(res, '图-')
    ...
    initData(data) {
      this.chartObj = new Pie('containerBusinessOpportunityBOX', {
        appendPadding: 10,
        data,
        angleField: 'num',
        colorField: 'name',
...
#containerBusinessOpportunityBOX {
  height: 248px;
