...
// 根据path获取moduleId，用于跳转菜单提交流程
// 不能直接通过路由获取，其他页面引用组件就会是其他页面的moduleId
export function getModuleByPath(query) {
  return request({
    url: '/app/user/module/path',
    method: 'get',
    params: query
  })
}
...
// 设置部门分管领导
export function addDeptPrincipalLeader(params) {
  return request({
    url: '/system/dept/add/principal/leader',
    method: 'get',
    params
  })
}
...
// 查询部门分管领导
export function deptLeaderList(params) {
  return request({
    url: '/system/dept/dept/leader',
    method: 'get',
    params
  })
}
...
// 查询部门主管
export function deptPrincipalList(params) {
  return request({
    url: '/system/dept/dept/principal',
    method: 'get',
    params
  })
}
